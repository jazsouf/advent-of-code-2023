...
export default async function Day3() {
	...
	engineDataWrapped.forEach((line, Ypos) => {
		...
		line.split('').forEach((char, Xpos) => {
			...
			if (isNumber(char)) {
				...
				y = Ypos
				surrounding =
					cloneData[Ypos][Xpos - 1] +
					cloneData[Ypos - 1][Xpos - 1] +
					cloneData[Ypos + 1][Xpos - 1] +
					cloneData[Ypos + 1][Xpos] +
					cloneData[Ypos - 1][Xpos]
				if (isNumber(cloneData[Ypos][Xpos + 1])) {
					...
					if (isNumber(cloneData[Ypos][Xpos + 2])) {
						...
						xEnd += 1
						surrounding +=
							cloneData[Ypos + 1][xEnd] +
							cloneData[Ypos - 1][xEnd] +
							cloneData[Ypos][xEnd + 1] +
							cloneData[Ypos + 1][xEnd + 1] +
							cloneData[Ypos - 1][xEnd + 1]
					} else {
						surrounding +=
							cloneData[Ypos][xEnd + 1] +
	...
	const filteredNumber = numbersLocations.filter((numLoc) =>
		numLoc[1].split('').some((char: string) => char !== '.')
	)

	const properNumbersSum = filteredNumber
		.map((numLoc) => Number(numLoc[0]))
		.flat()
	...
	const numbersNearGear = numbersLocations.filter((numLoc) =>
		numLoc[1].split('').some((char: string) => char === '*')
	)
	...
	const ratiosSum = doubles
		.map((doubles) => Number(doubles[0]) * Number(doubles[1]))
		.reduce((acc, curr) => acc + curr, 0)
